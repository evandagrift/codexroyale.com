...
# Overview
#### [Codexroyale.com](https://codexroyale.com/) tracks searched Clash Royale player's game results and other game data. This data is then presented to end users on the site. The Clash Royale API only provides a player's past ~30 battles at any point in time, so searched players are continuously tracked in a seperate thread to ensure all future battles are saved. When players are searched for the first time, they are added to a high priority list that updates before any other tracked player so new player battles will be retreivable quickly. 
